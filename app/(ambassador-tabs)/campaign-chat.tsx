...
export default function ChatScreen() {
  ...
  const [notificationProgress, setNotificationProgress] = useState({
    ...
    current: 0,
    total: 0,
    status: "Preparing notifications...",
  });
  ...
  // Ambassador's assigned campaign
        ...
        try {
          ...
          if (userDoc.exists()) {
            ...
            if (userData) {
              ...
              setAssignedCampaignId(userData.assignedCampaignId || null);
            }
          }
        } catch (error) {
  ...
  // Fetch campaign participants when assignedCampaignId is available
  useEffect(() => {
    ...
    const fetchCampaignData = async () => {
      try {
        const campaignDoc = await getDoc(
          doc(db, "campaigns", assignedCampaignId)
        );
        if (campaignDoc.exists()) {
          ...
          setCampaignParticipants(uniqueParticipants);
        }
      } catch (error) {
    ...
    fetchCampaignData();
  }, [assignedCampaignId, db]);

  // Listen for messages in real-time
  useEffect(() => {
    if (!currentUser || !assignedCampaignId) return;

    const messagesRef = collection(db, "campaigns", assignedCampaignId, "chat");
    ...
    return () => unsubscribe();
  }, [currentUser, assignedCampaignId, blockedUsers]);
  ...
  useEffect(() => {
    ...
    messages.forEach((msg) => {
      const msgRef = doc(db, "campaigns", assignedCampaignId!, "chat", msg.id);
      ...
      if (docSnap.exists()) {
        ...
        if (data?.messageId) {
          ...
          if (pinned) {
            ...
            getDoc(
              doc(db, "campaigns", assignedCampaignId, "chat", data.messageId)
            ).then((msgDoc) => {
              if (msgDoc.exists()) {
                setPinnedMessage({
                  id: msgDoc.id,
    ...
    const unsubscribe = onSnapshot(
      ...
      (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
          ...
          // For allUsers, include all users
          allUsersData.push({
            id: doc.id,
  ...
  const sendMessage = async () => {
    if (!inputText.trim() || !currentUser || !assignedCampaignId) return;

    try {
      const messageText = inputText.trim();

      // Check if message contains @everyone and user is admin/ambassador
      const isCurrentUserAdmin =
        currentUserData?.isAdmin ||
        currentUserData?.role === "admin" ||
        currentUserData?.isAmbassador;
      ...
      await addDoc(
        collection(db, "campaigns", assignedCampaignId!, "chat"),
        messageData
      );
  ...
  // Send @everyone notifications to all users
  const sendEveryoneNotifications = async (messageText: string) => {
    try {
      const senderName =
        currentUserData?.name || currentUser?.displayName || "Admin";

      // Get all users with push tokens (excluding current user)
      const usersWithTokens = allUsers
        .filter(
          (user) =>
            user.expoPushToken &&
            user.id !== currentUser?.uid &&
            !user.adminBlockedUsers?.includes(currentUser?.uid)
        )
        .map((user) => user.expoPushToken);

      if (usersWithTokens.length === 0) {
        Alert.alert("No Users", "No users found with notification tokens.");
        return;
      }
      ...
      // Get first 1-2 lines of message (up to 100 chars)
      ...
      for (let i = 0; i < usersWithTokens.length; i += batchSize) {
        ...
        setNotificationProgress((prev) => ({
          ...prev,
          current: i,
          status: `Sending notifications... (${i}/${usersWithTokens.length})`,
        }));
      ...
      setNotificationProgress((prev) => ({
        ...prev,
        current: usersWithTokens.length,
        status: `Completed! Sent to ${successCount} users${
      ...
      setTimeout(() => {
        setNotificationProgress((prev) => ({ ...prev, visible: false }));
      }, 2000);
    } catch (error) {
      console.error("Error sending @everyone notifications:", error);
      setNotificationProgress((prev) => ({
        ...prev,
        status: "Error sending notifications",
      }));
  ...
  const handleInputChange = (text: string) => {
    ...
    if (lastAtIndex !== -1) {
      ...
      if (queryText.length > 0 && spaceIndex === -1) {
        ...
        // Add @everyone suggestion for admins/ambassadors
        const isCurrentUserAdmin =
          currentUserData?.isAdmin ||
          currentUserData?.role === "admin" ||
          currentUserData?.isAmbassador;
  ...
  const handleReact = async (messageId: string, emoji: string) => {
    if (!currentUser || !assignedCampaignId) return;
    const msgRef = doc(db, "campaigns", assignedCampaignId, "chat", messageId);
  ...
  const handlePinMessage = async (messageId: string) => {
    ...
    const isCurrentUserAmbassador = currentUserData?.isAmbassador;

    if (!isCurrentUserAdmin && !isCurrentUserAmbassador) {
      Alert.alert(
        "Permission Denied",
        "Only admins and ambassadors can pin messages."
      );
      return;
    }
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              // Check if current user is admin/ambassador
              const isCurrentUserAdmin =
                currentUserData?.isAdmin ||
                currentUserData?.role === "admin" ||
                currentUserData?.isAmbassador;
        ...
        [
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: async () => {
              try {
                await deleteDoc(
                  doc(db, "campaigns", assignedCampaignId!, "chat", messageId)
                );
              } catch (e) {
          ...
          >
            ...
            >
              ...
              >
                ...
                {/* Admin/Ambassador pin button */}
                {(isCurrentUserAdmin || currentUserData?.isAmbassador) && (
                  <TouchableOpacity
                    style={{
                      marginLeft: 8,
                      marginRight: 2,
                ...
                {/* Admin/Ambassador delete button */}
                {(isCurrentUserAdmin || currentUserData?.isAmbassador) && (
                  <TouchableOpacity
                    style={{
                      marginLeft: 8,
  ...
  return (
    <SafeAreaView edges={["top"]} className="flex-1 bg-white">
      ...
      >
          ...
          <View className="flex-row items-center">
            <View>
              <Text className="font-gBold text-lg text-gray-900">
                {campaignData?.title || "Campaign Chat"}
              </Text>
              <Text className="text-gray-500 text-sm">
                {campaignData?.type || "Ambassador Campaign Discussion"}
              </Text>
            </View>
          </View>
          {/* User count badge */}
              ...
              <View className="bg-white rounded-t-3xl p-6">
                <View className="flex-row items-center justify-between mb-4">
                  <Text className="font-gBold text-xl text-gray-900">
                    Campaign Participants ({campaignParticipants.length})
                  </Text>
                  <TouchableOpacity
                ...
                <FlatList
                  data={allUsers.filter((user) =>
                    campaignParticipants.includes(user.id)
                  )}
                  keyExtractor={(item) => item.id}
                  showsVerticalScrollIndicator={false}
                  ...
                  renderItem={({ item }) => {
                    ...
                    const isAmbassador = item.isAmbassador;
        ...
        <Modal
          visible={notificationProgress.visible}
          animationType="fade"
          transparent={true}
          onRequestClose={() => {}} // Prevent closing during notification sending
        >
          <View className="flex-1 bg-black/50 justify-center items-center">
            <View className="bg-white rounded-2xl p-6 w-11/12 max-w-md">
              <View className="items-center">
                ...
                <View className="w-full bg-gray-200 rounded-full h-2 mb-4">
                  <View
                    ...
                    style={{
                      width: `${
                        notificationProgress.total > 0
                          ? (notificationProgress.current /
                              notificationProgress.total) *
                            100
                          : 0
                      }%`,
                    }}
                ...
                <Text className="text-gray-500 text-sm">
                  {notificationProgress.current} / {notificationProgress.total}{" "}
                  users
                </Text>

                {/* Don't show close button until completed */}
                {notificationProgress.current >= notificationProgress.total && (
                  <TouchableOpacity
                    onPress={() =>
                      setNotificationProgress((prev) => ({
                    ...
                    className="bg-gray-200 rounded-xl py-3 px-6 items-center mt-4"
                  >
                    <Text className="text-gray-700 font-gBold text-base">
                      Close
                    </Text>
